...
type TailorForm = z.infer<typeof schema>
...
const TailorResumePage = () => {
  ...
  const onSubmit = async (data: TailorForm) => {
    setError(null)
    setResult('')
    try {
      const response = await aiService.tailorResume({
  ...
  return (
    <AppShell title="Tailor Resume" subtitle="Match your resume to a job description" showSearch={false} showActions={false}>
          ...
          <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g., focus on leadership and product strategy"
              />
            </div>
            <button
        ...
        <div className="bg-card-light dark:bg-card-dark rounded-2xl shadow-card border border-gray-100 dark:border-gray-800 p-6 flex flex-col">
          ...
          <div className="flex-1 bg-gray-50 dark:bg-gray-800/60 rounded-xl p-4 text-sm overflow-y-auto min-h-[300px] max-h-[500px]">
            {isSubmitting ? (
              ...
              <div className="flex flex-col items-center justify-center h-full gap-3 text-text-secondary">
                <span className="material-symbols-outlined text-4xl text-gray-300 dark:text-gray-600">auto_awesome</span>
                <p className="text-sm text-center">Generated content will appear here<br/>once the AI finishes processing.</p>
              </div>
            )}
          </div>
        </div>
